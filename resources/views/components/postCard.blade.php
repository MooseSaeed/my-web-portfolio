<article id="post-card"
    class="text-white transition-colors py-6 px-5 flex flex-col justify-between duration-300 
            ...
            <span class="mt-4 block text-gray-400 text-xs">
                Published <time>{{ $post->created_at->diffForHumans() }}</time> in <span class="space-x-2">
                    <a href="/?category={{ $post->category->slug }}"
                        class="ml-1 px-6 py-1 border border-violet-400 rounded-full text-violet-400 text-xs uppercase font-semibold"
